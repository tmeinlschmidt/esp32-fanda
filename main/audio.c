// audio.c — sine tone synth over the new I2S std driver.
// Generates samples in a small chunk buffer and streams via i2s_channel_write
// until the requested duration elapses. Phase accumulator carries across
// chunks so there are no boundary clicks. sinf() per sample is fine for these
// short tones; if CPU ever mattered, a small LUT would replace it.
#include "audio.h"
...
// Tone parameters. Amplitude is 0.5 of int16 max to leave ~6 dB headroom —
// MAX98357A + a cheap speaker can clip/buzz nastily near full scale.
